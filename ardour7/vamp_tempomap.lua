...
function factory () return function ()
    ...
    for r in sel.regions:regionlist ():iter () do
            ...
            if fl then
                ...
                for f in fl:iter () do
                    ...
                    if f.hasTimestamp then
                        local sample_no_in_region = f.timestamp
                        local region_begin = r:position():samples()
                        local tempo_change_location = region_begin + sample_no_in_region
                        table.insert (beats[r:name ()], {pos = tempo_change_location, beat = f.label})
                    end
                end
            end
        ...
        vamp:analyze (r:to_audioregion():to_readable (), 0, callback)
    ...
    for n,o in pairs(beats) do
        ...
        for _,t in ipairs(o) do
            ...
            local location = t['pos']
            if  last_bpm ~= bpm then
                last_bpm = bpm
        if tempo_count < 4 then
            tempo_count = tempo_count + 1
            tempo = Temporal.Tempo(bpm, bpm, 4.0)
	    location = location
            tm:set_tempo(tempo, location)
            add_undo = true
            print(sample, bpm)
        end
            end
    ...
    print("Out of loop with ", tempo_count, " entries")
